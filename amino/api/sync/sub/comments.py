...
class CommunityCommentsModule(BaseClass):
	...
	def upvote_comment(self, blogId: str, commentId: str):
		...
		**Parameters**
		...
		return self.req.make_sync_request("POST",  f"/x{self.comId}/s/blog/{blogId}/comment/{commentId}/vote?cv=1.2&value=1", data).json()
	...
	def unvote_comment(self, blogId: str, commentId: str):
		...
		**Parameters**
		...
		- commentId : ID of the Comment.
		"""
		return self.req.make_sync_request("DELETE",  f"/x{self.comId}/s/blog/{blogId}/comment/{commentId}/vote?eventSource=PostDetailView").json()
	...
	def reply_wall(self, userId: str, commentId: str, message: str):
		...
		**Parameters**
		...
		return self.req.make_sync_request("POST",  f"/x{self.comId}/s/user-profile/{userId}/comment", data).json()



	def get_wall_comments(self, userId: str, sorting: str = args.Sorting.Newest, start: int = 0, size: int = 25):
		"""
		...
		**Parameters**
		...
		return self.req.make_sync_request("GET", f"/x{self.comId}/s/user-profile/{userId}/comment?sort={sorting}&start={start}&size={size}").json()["commentList"]
